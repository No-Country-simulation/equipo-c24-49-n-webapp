...
export async function GET(request: Request) {
  try {
    ...
    const hasAccess = project.creator.toString() === session.user._id || 
...
export async function POST(request: Request) {
  try {
    ...
    const { title, description, category: categoryId, dueDate, assignedTo } = await request.json();
    ...
    const project = category.project as any;
    const isAllowed = project.creator.toString() === session.user._id || 
      project.collaborators.some((c: any) => 
...
export async function PUT(request: Request) {
  try {
    ...
    const isAdmin = project.collaborators.some((c: any) => 
      c.user.toString() === session.user._id && c.role === "admin"
    );
    const isEditor = project.collaborators.some((c: any) => 
      c.user.toString() === session.user._id && c.role === "editor"
    );
    ...
    if (!canEditAll && isAssignedUser && Object.keys(data).every(k => k === "status")) {
